...
- `is_existing_member: false` (新朋友) → `name`, `sex`, `university`, `birthday`, `mbti` and
  `want_membership` (`想` / `未需要住`) are filled; `instagram` blank.
- Always present: `occupation`, `consent`, and the event/meta keys. (`want_membership` is only
  asked of new members; it is sent as an empty string for existing members.)
