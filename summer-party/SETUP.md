...
- `is_existing_member: false` (新朋友) → `name`, `sex`, `whatsapp`, `university`, `birthday`, `mbti`,
  `instagram` and `want_membership` (`想` / `未需要住`) are filled.
- Always present: `instagram`, `occupation`, `consent`, and the event/meta keys. (`whatsapp` and
  `want_membership` are only asked of new members; sent as empty strings for existing members.)
...
| `WhatsApp`      | Phone / Single line text    | `whatsapp`            |
